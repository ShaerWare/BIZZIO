...
## #280 — Коммерческий аукцион: нормализация критериев в расчёте рейтинга (2026-08-13)

**Проблема:** критерии «Срок» и «Аванс» влияли на исход непропорционально своим весам. Нормировка шла от всего исходного значения критерия (0 баллов при исходном значении, 100 — при нуле), поэтому снижение аванса на 10 п.п. перекрывало заметное снижение цены даже при весах 80/10/10 (см. протокол А-260812-0001).

**Решение (по ТЗ v1.0 от 12.08.2026):** нормировка ведётся не до нуля, а до **расчётной границы полного балла** — системной предустановки, организатор её не задаёт:

- цена — полный балл при снижении на 40% от НМЦ (`full = P0 × 0,6`);
- срок — при сокращении вдвое (`full = T0 × 0,5`);
- аванс — при снижении до 0% (`full = 0`).

Формула критерия: `балл = вес × clamp((ref − x) / (ref − full), 0, 1)`. Веса, суммирование баллов, логика Лучшего предложения и непрерывного лидерства не менялись. Частный случай ТЗ: при `A0 = 0%` критерий считается достигшим границы — полный балл без деления на ноль.

Пересчитаны и производные величины: пороги-подсказки «Уменьшите до …» (инверсия новой нормировки: `x = ref − score/100 × (ref − full)`), проверка превосходства над лидером, доступность кнопки «Подать предложение». Клиентское зеркало формулы в Alpine получает границы с сервера (`fullRefs()`), чтобы предпросмотр рейтинга совпадал с серверной проверкой. Добавлена подсказка «Макс. балл по критерию» при достижении границы.

Исторические процедуры не пересчитываются: протоколы и таблицы результатов читают сохранённые `score_*`/`total_score`.
...
**Дополнение (2026-08-14):** пуш в зеркало `BizzioDev/BIZZIO` не создаёт workflow run — после реального пуша `gh api repos/BizzioDev/BIZZIO/actions/runs` отдаёт только ран от `workflow_dispatch` (воркфлоу при этом `active`, ручной запуск и деплой на тест из зеркала отрабатывают полностью успешно). Поэтому в скилле деплой в зеркале запускается явно: `gh workflow run "CI/CD" --repo BizzioDev/BIZZIO --ref develop -f target=test`.

## Инфраструктура: серверы переключены на репу заказчика + фикс молчаливого деплоя старого кода (2026-08-14)

**Deploy keys включены владельцем организации.** Read-only ключ сервера `~/.ssh/bizzio_repo` добавлен в BizzioDev/BIZZIO → Settings → Deploy keys, `origin` в `/var/www/bizzio-test` и `/var/www/BIZZIO` переключён с `https://github.com/ShaerWare/BIZZIO.git` на `git@github.com:BizzioDev/BIZZIO.git`. Теперь код на оба контура приходит из репы заказчика. Ограничение по environment осталось: `required reviewers` на free-плане для приватной репы по-прежнему отдаёт 422, поэтому авто-деплой прода живёт в ShaerWare (там гейт работает).

**Ошибка, которую это вскрыло.** Прод простоял на релизе от 8 августа (`e8f97ce`) двое суток: job `Deploy main -> bizzio.ru` висел в статусе `waiting` — мердж в `main` прошёл, проверки зелёные, но раскатка ждала подтверждения в Environment `production`, которого никто не дал. Раскатано после явного указания владельца: сначала PR #281 (`5bf6705`, миграции таблиц чата процедуры применились), затем PR #292 (`1519ea2`) с #280 и инфраструктурой зеркала.

**Вторая, более опасная ошибка — деплой старого кода с зелёным статусом.** После переключения `origin` на зеркало деплой-job делает `git fetch origin main && git reset --hard origin/main`, а зеркало пушилось шагом ПОСЛЕ деплоя. В результате раскатка PR #292 отработала «успешно», но оставила прод на предыдущем коммите — падения нет, статус зелёный, кода нет. Исправлено с двух сторон:

- `ci.yml`: оба деплой-джоба перед `reset --hard` ждут появления `${{ github.sha }}` в зеркале (опрос до 10 минут) и падают с явным сообщением, если коммит так и не приехал;
...
## #282 Меню «Закупки»: размещение только через двухэтапную закупку (2026-08-20)

**Что сделано.** Из меню «Закупки» убраны пункты «Создать аукцион» и «Создать запрос цен», пункт «Создать коммерческий аукцион» переименован в «Создать закупку» (ведёт на `rfqs.create?procedure=commercial` — двухэтапная процедура #179). Пункт «Правила проведения» переименован в «Правила закупок». Новый порядок пунктов по ТЗ задачи: Найти закупку → Создать закупку → Мои закупки → Мои приглашения → Мои заявки → Правила закупок. Изменения внесены в обе версии меню — десктопный dropdown и раскрывающуюся группу мобильного меню.

Заодно приведены в соответствие кнопки размещения на страницах закупок: выпадающая кнопка «Разместить» с выбором «Запрос цен / Аукцион» на `/tenders` и `/my-tenders` заменена одной кнопкой «Создать закупку» — иначе скрытые в меню процедуры оставались доступны в два клика на тех же страницах. Текст пустого состояния на `/my-tenders` — «Разместите свою первую закупку».

**Что НЕ трогали.** Роуты `rfqs.create` и `auctions.create` и старые страницы `/rfqs`, `/auctions` с их кнопками создания оставлены рабочими: процедуры не удаляются, они лишь убраны из навигации. Заголовок страницы правил («Правила проведения тендеров») не менялся.

**Тесты:** `tests/Feature/ProcurementMenuTest.php` — 6 кейсов (состав и порядок пунктов, ссылка на коммерческую процедуру, отсутствие пунктов организатора у пользователя без компании, кнопки на `/tenders` и `/my-tenders`). Полный прогон: 442 passed.

**Изменённые файлы:** `resources/views/layouts/navigation.blade.php`, `resources/views/tenders/index.blade.php`, `resources/views/tenders/my-tenders.blade.php`, `tests/Feature/ProcurementMenuTest.php` (новый).

## #283 НМЦ скрывается из общего доступа после завершения закупки (2026-08-20)

**Что сделано.** Добавлен `Auction::startingPriceHiddenFor(?User $user)`: у процедуры в статусе `closed` (для двухэтапной закупки — после закрытия этапа 2 и формирования протокола) НМЦ показывается только организатору и участникам закупки. «Участник» определяется методом `isProcedureInsider()` — модератор приглашённой компании (переиспользует `isParticipant()` из #237) **либо** компании, подавшей заявку/предложение: у открытой процедуры заявку подают без приглашения, и по одним `invitations` такой участник не нашёлся бы.

Скрытие применено в двух местах, где НМЦ видна публично: карточка `components/auction-card.blade.php` (списки `/tenders` и `/auctions`) — вместо суммы «скрыта»; страница `auctions/show.blade.php`, блок «Параметры аукциона» — «скрыта после завершения закупки».

**Утечек по другим каналам нет** (проверено): long-polling `AuctionController::getState()` отдаёт `nmc` только при `status = trading` и после закрытия возвращает 400; PDF-протокол с НМЦ доступен только организатору и участникам (A16); блок результатов на странице закрытой процедуры уже скрыт от посторонних (#237).

**Область применения.** Правило действует для обеих процедур — стандартного аукциона и двухэтапной закупки: в списке `/tenders` они соседствуют, и показывать НМЦ у одной завершённой процедуры, скрывая у другой, было бы непоследовательно. Пока процедура идёт (`active`/`trading`), НМЦ по-прежнему публична — без неё нельзя торговаться.
...
## #285 Автопоиск в фильтрах закупок, кнопка «Применить» убрана (2026-08-20)

**Что сделано.** Фильтры на `/tenders` применяются сами: текстовые поля отправляют форму с debounce 600 мс (`@input.debounce.600ms="$el.form.requestSubmit()"`), выпадающие списки — по `@change`, выбор компании-организатора из подсказок (#284) — сразу после клика. Кнопка «Применить» удалена, «Сбросить» осталась и растянута на всю ячейку. Форма помечена атрибутом `data-autofilter`, обёрнута в `x-data="{}"` — без этого Alpine-директивы на полях вне компонента не работают.

Два момента, которые пришлось решить помимо самого автосабмита:
- **Фокус.** Перезагрузка страницы сбивает курсор, и печатать дальше нельзя. Перед отправкой имя активного поля и позиция курсора сохраняются в `sessionStorage`, после загрузки восстанавливаются.
- **Мусор в URL.** При автосабмите в строку запроса попадали все пустые фильтры (`?search=&kind=&status=…`). Пустые поля отключаются (`disabled`) в обработчике `submit`, поэтому в URL остаются только заданные значения.

**Тесты:** `tests/Feature/TenderAutoFilterTest.php` — 5 кейсов (кнопки «Применить» нет, «Сбросить» на месте, форма помечена для автопоиска, фильтр по статусу и поиск по названию продолжают работать обычным GET-запросом). Полный прогон: 463 passed.

**Ошибка по пути:** тест `assertDontSee('Применить')` падал из-за JS-комментария в `@push('scripts')` — комментарии в блоке `<script>` попадают в HTML, в отличие от blade-комментариев `{{-- --}}`. Формулировка комментария изменена.

**Изменённые файлы:** `resources/views/tenders/index.blade.php`, `tests/Feature/TenderAutoFilterTest.php` (новый).

## #287 Организационно-правовая форма перед названием компании (2026-08-21)

**Что сделано.** Блок «Организационно-правовая форма» перенесён перед полем «Название компании» — в форме создания (`companies/create.blade.php`) и в форме редактирования (`companies/edit.blade.php`). У поля названия появились placeholder «Ромашка» и подсказка «Без организационно-правовой формы и кавычек».

Одного перестановкой проблема из задачи («пользователи упорно пишут ООО и ставят кавычки в названии») не решается, поэтому добавлено правило `App\Rules\CompanyNameWithoutLegalForm`: название не должно начинаться с ОПФ (ООО, АО, ПАО, ЗАО, ОАО, ИП, НКО, АНО, ФГУП, МУП, ГУП, ТОО) и содержать кавычки. Сообщения разные для двух случаев — правило-класс вместо `not_regex` именно ради этого. Регулярка требует после ОПФ разделитель (`[\s".«]`) или конец строки, поэтому «Ипотека Плюс» проходит.

Это важно для протоколов: `Company::legalNameWithInn()` (#191) собирает наименование из двух полей — `ООО «Ромашка» (ИНН …)`. Если ОПФ и кавычки лежат в названии, получается `ООО «ООО "Ромашка"»`.

**Ошибка, вскрытая тестами.** Правило, повешенное на `UpdateCompanyRequest` безусловно, ломало редактирование компаний, у которых ОПФ уже в названии: `CompanyDocumentsTest` упал на сохранении документов (фабрика генерирует названия вида «ООО …», как и реальные компании, заведённые до этой задачи). Правило теперь применяется только когда название реально меняется — остальные поля таких компаний сохраняются свободно.
...
## #295 Чат закупки: вопрос можно задать до подачи заявки (2026-08-21)

**Задача противоречила исходной логике #218,** где чат видели только участники — подавшие заявку или приглашённые. Разрешение сделано точечным, чтобы не разрушить приватность закрытых процедур:

- Добавлен `chatOpenToProspects()` — истина только для **открытой** процедуры (`type = open`) и только пока идёт этап 1. У закрытой процедуры состав по-прежнему ограничен приглашёнными: там приватность и есть смысл процедуры.
- `chatCompaniesFor()` при открытом чате возвращает любые компании пользователя (кроме отстранённых и компании-организатора — она участвует в чате как организатор). После закрытия этапа 1 доступ сохраняют те, кто уже писал: их компании берутся из `procedure_participants`, поэтому переписка не пропадает у автора вопроса.
- Обезличивание не изменилось: код «У-01» выдаётся при первом сообщении, названий компаний участники по-прежнему не видят.
- Отстранение работает для таких компаний без изменений: `ban()` создаёт запись участника сам, а блокировка подачи заявки уже проверялась через `isCompanyBanned()` в `RfqController` и `AuctionController`. Добавлен `chatVisibleCompanyIds()` — список для организатора теперь включает и тех, кто писал в чат, не подав заявку, иначе отстранить их из интерфейса было бы нельзя.
- В блоке чата появилась подсказка «Задать вопрос можно до подачи заявки на участие» — показывается только когда правило действует.

**Изменение поведения в существующем тесте.** `ProcedureChatTest::test_outsider_cannot_read_chat` проверял, что посторонняя компания получает 403 на открытой процедуре — теперь это разрешено. Тест переписан на закрытую процедуру (`test_outsider_cannot_read_chat_of_closed_procedure`) и дополнен проверкой, что гость без авторизации в чат по-прежнему не попадает.

**Тесты:** `tests/Feature/ProcedureChatProspectTest.php` — 8 кейсов (вопрос без заявки, выдача кода, скрытие названий компаний, закрытая процедура остаётся по приглашениям, организатор видит и банит компанию без заявки, забаненный не пишет, автор вопроса сохраняет историю после закрытия этапа 1, компания без сообщений доступ теряет).

**Изменённые файлы:** `app/Traits/HasProcedureChat.php`, `resources/views/partials/procedure-chat.blade.php`, `tests/Feature/ProcedureChatTest.php`, `tests/Feature/ProcedureChatProspectTest.php` (новый), `CLAUDE.md`.

## #273 Канбан продублирован в организацию BizzioDev (2026-08-21)

Изменений в коде нет — задача инфраструктурная, выполнена через GitHub API.

- `gh project copy 4 --source-owner ShaerWare --target-owner BizzioDev` — создан приватный org-проект **БИЗЗИО**, https://github.com/orgs/BizzioDev/projects/1. Скопирована вся структура: колонки Status (Backlog / Первоочередные / In review (test) / Правки после тестов / Ready to prod / Done), поля Priority, Size, Estimate, Start date, Target date.
- Все **161 карточку** воссозданы как **draft issues** с исходными заголовком, телом и значениями Status/Priority. Распределение по колонкам совпадает с оригиналом (Done 137, Backlog 11, In review 9, Первоочередные 2, Правки 1, Ready to prod 1).

**Почему draft, а не issues.** Карточка Projects — это ссылка на issue, а зеркало `BizzioDev/BIZZIO` держит только код, issues там нет. Копирование 161 issue в зеркало перенумеровало бы их (#273 → #1..#161) и сломало ссылки вида `#295` в коммитах и `CLAUDE.md`. Draft-карточки самодостаточны и ничего не ломают; в конце тела каждой добавлена строка `исходная задача: ShaerWare/BIZZIO#NNN` для трассировки.

**Ограничения переноса:** комментарии к issues, метки, привязанные PR и связь «карточка ↔ issue» не переносятся — у draft-элементов этих полей нет. Оригинальная доска https://github.com/users/ShaerWare/projects/4 не изменялась.

**Ошибка по пути:** `json.load` на дампе `gh project item-list` падал с `UnicodeDecodeError: 'charmap' codec` — Python на Windows открывает файл в cp1251, а gh пишет UTF-8. Файлы с русским текстом надо открывать через `io.open(..., encoding='utf-8')`, а для вывода в консоль — `sys.stdout.reconfigure(encoding='utf-8')`.

## #181 Новая главная: перенос согласованной вёрстки v26 (2026-08-21)

**Что сделано.** `/` стал единой точкой входа: `HomeController@index` отдаёт гостевой или авторизованный вариант по состоянию сессии. Прежний `/dashboard` заменён редиректом на корень (имя роута `dashboard` сохранено ради старых ссылок и закладок), ссылки в коде и меню переведены на `route('home')`.

**Вёрстка.** Эталон `Bizzio_Dashboard_v26` — самостоятельный прототип на своём CSS с фиксированным кадром (1680×945 / 1024×1366 / 393×852) и фото-заглушками из спрайта. Перенесён так:

- CSS шести экранов собран в `resources/css/v26.css` (отдельный Vite-вход вместе с `resources/js/v26.js`): десктоп-база → планшетные переопределения → мобильная секция (в макете она уже скоупирована `#bizzio-mobile-v1`). Отдельный вход нужен потому, что селекторы прототипа (`.page`, `.card`, `.service`) слишком общие, чтобы жить рядом с Tailwind остальных страниц.
- Перенесённые секции намеренно не редактировались — их можно сверять с эталоном; всё, что понадобилось для боевой страницы, дописано отдельной секцией PRODUCTION в конце файла: кадр прототипа превращён в тянущуюся по контенту страницу, добавлено переключение десктоп/мобильной разметки на 768px, фото-спрайты заменены реальными изображениями.
- Десктоп и планшет — одна разметка (различаются медиазапросами), мобильный экран в макете имеет собственную структуру, поэтому в шаблонах он идёт вторым блоком.
- Иконки макета собраны в спрайт `partials/v26/icons.blade.php` (43 символа), PNG-иконки и картинка города — в `public/images/v26/`. Логотип взят существующий, из `public/images`.

**Данные.** Авторизованная главная показывает те же виджеты, что и прежний дашборд: `DashboardController::index()` разделён на тонкую обёртку и `dashboardData(User)`, который теперь вызывает и `HomeController`. Новых сущностей, таблиц и API задача не добавила.

**Аналитика (ТЗ, раздел 4).** Элементы без реализации остаются видимыми и кликабельными, но никуда не ведут: `data-inactive-feature` и `data-future-service` шлют цели Яндекс.Метрики `inactive_feature_click` и `future_service_interest` с параметрами `placement`, `auth_state`, `device_type`. Один физический клик = одно событие: кнопка «Мне интересно» внутри карточки останавливает всплытие, иначе ушло бы два. Персональные данные не передаются. Счётчик уже был подключён (id 106718528), новой инфраструктуры не потребовалось.

**AUTH_URL (открытый вопрос ТЗ).** По решению владельца оставлены существующие `/login` и `/register`, но адреса вынесены в конфигурацию (`config('app.auth_url')`, `config('app.register_url')`, переменные `AUTH_URL` / `REGISTER_URL`) — как того требует ТЗ, в компонентах они не зашиты.

**Изменения в существующих тестах.** `DashboardTest` проверял `assertViewIs('dashboard')` на `/dashboard` — переписан на новую главную плюс отдельная проверка редиректа со старого адреса. `SubscriptionTest` (лента постов) переведён на `route('home')`. `ProcurementMenuTest` проверял меню Tailwind-темы на `/dashboard`, где этой темы больше нет, — переключён на `/tenders`, где меню и живёт.

**Тесты:** `tests/Feature/HomeV26Test.php` — 6 кейсов (гостевой и авторизованный варианты, ссылки входа/регистрации из конфига, разметка для аналитики, лента постов на главной).

**Изменённые файлы:** `app/Http/Controllers/HomeController.php` (новый), `app/Http/Controllers/DashboardController.php`, `app/Http/Controllers/{PostController,Auth/SocialiteController}.php`, `config/app.php`, `routes/web.php`, `resources/css/v26.css` (новый), `resources/js/v26.js` (новый), `resources/views/layouts/v26.blade.php` (новый), `resources/views/home/{guest,authorized}.blade.php` (новые), `resources/views/partials/v26/icons.blade.php` (новый), `resources/views/layouts/navigation.blade.php`, `vite.config.js`, `public/images/v26/*`, `CLAUDE.md`, тесты.

**Две регрессии, вскрытые полным прогоном (#181).** Первая: прежний лендинг подключал SEO-партиал `partials/seo.blade.php` (#152 — Open Graph, Twitter Card, canonical, JSON-LD), а новый layout `layouts/v26.blade.php` — нет, и главная осталась без разметки для поисковиков. Партиал подключён, собственный `<meta name="description">` из layout убран как дублирующий. Вторая: `ExampleTest` дергает `/` без `RefreshDatabase` — раньше главная была статической вью, теперь она читает публичные закупки и проекты, поэтому тест падал на «no such table: rfqs». Трейт добавлен.

**Третья, самая важная находка (#181): вход и регистрация были встроены в лендинг.** `routes/auth.php` отдавал `GET /register` и `GET /login` не страницами, а редиректами на старую главную (`/?mode=register`, `/#login-form`) — формы жили внутри `welcome.blade.php`. С новой главной, где форм нет, это давало петлю: `/register` → `/` → кнопка → `/register`. То есть вход на сайт был бы сломан.
...
## Правки после тестов: #284, #296, #181 (2026-08-29)

Разобраны задачи из колонки канбана «Правки после тестов».

### #284 — обрезался выпадающий список организатора

Карточка фильтров на `/tenders` имела `overflow-hidden`, и абсолютно спозиционированный список подсказок обрезался её нижней границей. Заменено на `overflow-visible`; длинные названия компаний в подсказках получили `truncate`.

### #296 — документацию нельзя было скачать после завершения этапа 2

**Причина.** У коммерческого аукциона нет своих файлов: конкурсную документацию организатор загружает на этапе 1 (запрос цен), а `CommercialAuctionLauncherService` медиа в аукцион не копирует. Партиал `procurement-documents-list` рендерится только при непустом `allProcurementDocuments()`, поэтому на странице аукциона блок вообще не появлялся — оставался только протокол.

**Вторая, скрытая проблема:** срок хранения отсчитывался от `closed_at` самой процедуры. У этапа 1 это момент закрытия приёма заявок, то есть 30 дней начинали течь ещё до окончания торгов, а `documents:cleanup` мог удалить документацию идущей процедуры.

**Что сделано.**
- В трейт `HasProcurementDocuments` добавлены `procurementDocumentsHolder()` (носитель файлов) и `documentsRetentionStartedAt()` (начало срока хранения).
- `Auction::procurementDocumentsHolder()` для коммерческого аукциона возвращает связанный `Rfq` — страница этапа 2 и маршруты `auctions.documents.*` отдают документацию этапа 1.
- `Rfq::documentsRetentionStartedAt()` у двухэтапной процедуры берёт `closed_at` порождённого аукциона: пока торги идут, срок не начинается.
- `documents:cleanup` перед удалением спрашивает `documentsRetentionExpired()`, а не только сравнивает даты в SQL.

**Тесты:** `ProcurementDocumentsRetentionTest` — три новых случая: этап 2 отдаёт файлы этапа 1, документация переживает незакрытый этап 2, удаляется через 30 дней после его закрытия.

### #181 — расхождения с эталоном v26

Сверено с приложенным к таску архивом `Bizzio_Dashboard_v26`.

**Три системные причины, дававшие сразу несколько пунктов замечаний:**
1. `.page a{color:inherit}` стояло последним правилом `v26.css` и при равной специфичности перебивало цвета компонентов — отсюда «Открыть профиль» не синяя (1.2.6) и ссылки гостевых карточек не зелёные (3.1.3). Сброс перенесён в начало каскада.
2. У гостевой страницы не было класса `guest-page`, а селекторы панелей — `.guest-page[data-panel="menu"] …`. Поэтому «Меню» и «Сервисы» не открывались ни на десктопе (1.1.4), ни на планшете (3.1.1).
3. В эталоне `Войти`/`Регистрация` и крестики drawer-ов — `<button>`/lucide-иконки, у нас `<a>` и `<svg><use>`. Инлайновой ссылке не применяются `height`/`padding` (1.1.2, 3.1.2), а `<svg>` без размеров растягивается до 300×150 — отсюда «огромный крестик Выход» (2.1.2, 2.1.3). Добавлены `display:inline-flex` и явные размеры иконок.

**Остальное по пунктам.** Фавикон приведён к общему набору сайта (1.1.1). На странице входа логотип Laravel заменён на логотип Bizzio и добавлена ссылка на регистрацию (1.1.3). Быстрые действия авторизованной главной приведены к составу и PNG-иконкам эталона: Найти друзей / Найти закупку / Создать закупку / Настроить новости (1.1.5, 3.2.1). Блок «Мои закупки» заменён на «Актуальное в Bizzio» (1.2.4, 2.2.2), «Приглашения и заявки» — на блок «Реклама» (1.2.5). Из меню раздела убраны «Мои компании» и «Настройки профиля» (1.2.2), в меню сервисов добавлен блок «Bizzio в соцсетях» (1.2.3, 2.1.3). Поле поиска в шапке оформлено как в эталоне (1.2.7). В блок новостей добавлена ссылка «Настроить выдачу» (1.1.7, 2.1.5, 3.1.6). Мобильные партиалы переписаны по эталону: гостевой получил кнопку входа в шапке, блоки «Актуальное в Bizzio», «Реклама» и заглушки, из меню убраны «Публикации» и дублирующие кнопки входа (2.1.1–2.1.5); авторизованный — меню профиля в шапке, нижнюю навигацию эталона, форму публикации и ленту (2.2.1, 2.2.2).

**Новый источник данных.** `HomeController::personalEvents()` собирает «Актуальное в Bizzio» авторизованного пользователя из четырёх разделов: приглашения в закупки, входящие заявки в контакты, заявки на вступление в компании, проекты его компаний; при нехватке личных событий блок добирается публичными.

**Планшет.** В эталоне это отдельные файлы, где правая колонка разобрана скриптом. Так как десктоп и планшет у нас делят разметку, в `v26.js` добавлен `initTabletReflow()`: на 768–1279px «Актуальное в Bizzio» встаёт после «Добро пожаловать» (у гостя) или после карточки быстрых действий (у авторизованного), «Реклама» и «Новости» уходят в конец средней колонки; при возврате на десктоп порядок восстанавливается (3.1.4, 3.1.5, 3.2.2).

**Изменённые файлы:** `app/Traits/HasProcurementDocuments.php`, `app/Models/{Auction,Rfq}.php`, `app/Http/Controllers/{HomeController,ProcurementDocumentController}.php`, `app/Console/Commands/CleanupProcurementDocuments.php`, `resources/views/partials/procurement-documents-list.blade.php`, `resources/views/tenders/index.blade.php`, `resources/views/layouts/{v26,guest}.blade.php`, `resources/views/auth/login.blade.php`, `resources/views/home/{guest,authorized}.blade.php`, `resources/views/partials/v26/*` (добавлены `ad-card.blade.php`, `social-links.blade.php`), `resources/css/v26.css`, `resources/js/v26.js`, тесты `ProcurementDocumentsRetentionTest`, `HomeV26Test`, `MobileLayoutV26Test`.

## 2026-08-30 — Релиз на прод c8b71a6 (PR #311)

**Задача:** выкатить на bizzio.ru содержимое столбца «Ready to prod» (10 карточек).

**Что выяснилось при сверке доски с git:**
- #280 (формула этапа 2) уже была в `main` с PR #292 от 15.08 — на проде с того же дня, карточку просто не передвинули.
- #273 (перенос на BizzioDev) — инфраструктурная, коммитов нет.
- Реально требовали выкатки 8 задач: #282, #283, #284, #285, #287, #288, #295, #296.
- В `develop` дополнительно лежали #181 (новая главная v26, статус «Правки после тестов») и #286 (фикс обратной связи). Вырезать #181 нельзя: коммит `a146f46` смешивает правки #284, #296 и #181 в одном коммите + общий пересобранный `public/build`. По решению пользователя выкачен весь `develop`.

**Ход выкатки:**
1. PR #311 `develop` → `main`, проверки (Tests / Pint / Assets freshness) зелёные, смержен → `main` = `c8b71a6`.
2. `git push customer main` — зеркало `BizzioDev/BIZZIO` обновлено сразу после мержа (сервер тянет оттуда; в августе прод из-за отставшего зеркала двое суток стоял на старом релизе).
3. Прод-джоба встала на ручном аппруве в GitHub Environment `production`. По прямому указанию пользователя выкачено вручную на сервере: `git reset --hard origin/main` + `bash scripts/deploy.sh` под `flock /var/lock/bizzio-prod-deploy.lock`.
4. Перед выкаткой снят дамп БД: `/root/bizzio-prod-20260830-1026.sql.gz` (27 МБ). Откатный коммит — `9ed2ec6`.

**Миграция:** `2026_08_21_120000_add_closed_at_to_procedures` — nullable `closed_at` в `rfqs`/`auctions` + бэкофилл из `updated_at` для закрытых процедур. Аддитивная, обратимая. Применена, колонки на месте.

**Проверка после деплоя:** HEAD = `c8b71a6`; `/`, `/tenders`, `/login` → 200; отдаются ассеты `v26-*.css/js` (новая главная живая); supervisor — php-fpm, nginx, scheduler, worker RUNNING; в логе ни одной PHP-ошибки, 5xx нет.

**Доска:** 10 карточек Ready to prod → Done, соответствующие issue закрыты.

### Попутно обнаружено (не чинилось)

- **Форма обратной связи (#286).** Письма уходят на `ADMIN_NOTIFICATION_EMAIL` = `admin@bizzio.ru`, отправитель `noreply@bizzio.ru` (SMTP Beget). Ящики `notify@`/`support@` в коде не фигурируют вообще. До этого релиза на проде была старая реализация `Mail::raw()` без `try/catch` и логирования, поэтому причину «письма не приходят» по логам восстановить было нельзя. Теперь ошибки отправки пишутся в лог. Наиболее вероятная причина — отсутствующий ящик `admin@bizzio.ru`; проверять в панели Beget.
- **`LOG_LEVEL=error`** на прод и тесте — `Log::info('Обратная связь отправлена')` в лог не попадёт, видны будут только неудачи.
- **Раздут `storage/logs/laravel.log`:** прод 810 МБ, тест 440 МБ. Забит дампами SQL от RSS-парсера (`duplicate key ... returning "id"` по фиду Habr, ~3000 записей в сутки, каждая с полным текстом статьи). Ротации нет (`LOG_STACK=single`). Требует отдельной задачи: `updateOrCreate` вместо `insert` в парсере + переход на `daily`-канал.
- **Run 33304955421 остался в статусе `waiting`** — прод уже выкачен вручную, аппрув не нужен, run надо отклонить/отменить руками.

## #181 Правки по третьему комментарию заказчика (2026-08-30)

Второй комментарий (26.08) был закрыт коммитом `a146f46`. Здесь — замечания от 29.08.

- **Отступ в меню «Сервисы» (п. 2).** Заголовки «Доступные сервисы» и «Будущие сервисы» в гостевом меню шли классом `guest-drawer-section`, которого **нет в v26.css** — поэтому оставались без отступа слева. В эталоне это `guest-drawer-label`, а разделитель между блоками — `guest-drawer-rule`; добавлена и подпись `guest-drawer-sub`, как в макете.
- **Состав блока событий (п. 3).** Заказчик назвал «блоком Добавить в избранное» существующий блок «Актуальное в Bizzio» (блока с таким именем нет ни в эталоне, ни в коде — уточнено у владельца). Для авторизованного там уже собирались события из четырёх разделов; у гостя не хватало компаний — добавлены последние верифицированные. «Друзей» в гостевом варианте нет по определению: без сессии их не от кого считать.
- **Подпись поиска на планшете (п. 4).** Полная фраза «Компании, проекты, люди, закупки и новости» не помещалась в поле и вылезала за границы. Добавлены `data-placeholder-full` / `data-placeholder-short`, а `initSearchPlaceholder()` в `v26.js` подставляет короткий вариант «Поиск в Bizzio» при ширине ≤1279px (планшет и уже) и возвращает полный на десктопе. Механизм работает и для `<input>` авторизованной шапки, и для `<span>` гостевой.

**Пункт 1 (новое меню на всех страницах, миграция вложенных меню второго уровня в меню раздела) в этот PR не входит** — по решению владельца выделен в отдельный этап: он затрагивает весь сайт, а не главную.

**Изменённые файлы:** `app/Http/Controllers/HomeController.php`, `resources/views/home/{guest,authorized}.blade.php`, `resources/js/v26.js`, `docs/CHANGELOG_CLAUDE.md`.

## #181 Новое меню на всех страницах сайта (2026-08-30)

Пункт 1 третьего комментария заказчика: «новое меню распространить на все страницы и разделы, старые вложенные меню второго уровня должны мигрировать в меню раздела».

**Что сделано.**

- **Шапка v26 стала общей.** `layouts/app.blade.php` (её используют 38 вью) больше не подключает Tailwind-навигацию `layouts/navigation.blade.php` — вместо неё `partials/v26/chrome.blade.php`: верхняя панель, панели «Меню» и «Сервисы», мобильная шапка и нижняя навигация. Файл старой навигации удалён, чтобы не осталось двух источников меню.
- **Стили шапки скоупированы.** Полный `v26.css` подключать на внутренние страницы нельзя: у прототипа очень общие селекторы, а в шаблонах сайта уже есть свои `card` (22 вхождения), `service` (71), `link` (23) — стили поехали бы по контенту. Поэтому правила шапки выделены в `resources/css/v26-chrome.css`, где каждый селектор ограничен `.v26-chrome`; мобильная часть уже изолирована скоупом `#bizzio-mobile-v1` в самом эталоне.
- **Вложенные меню второго уровня переехали.** Прежние выпадающие «Закупки» (Найти / Создать / Мои закупки / Мои приглашения / Мои заявки / Правила) и «Новости» (Лента / Ключевые слова) стали разделами меню раздела.
- **Один источник состава меню.** `partials/v26/menu-items.blade.php` и `partials/v26/services-items.blade.php` используются и шапкой внутренних страниц, и обеими версиями главной, и мобильными партиалами — списки не могут разъехаться. Классы пунктов передаются параметрами, потому что у десктопной и мобильной вёрстки эталона они разные.
- **Отступы под фиксированную мобильную шапку.** На главной их даёт обёртка `.bz-phone`, у внутренних страниц контент свой — добавлена обёртка `.v26-page-body` с отступами сверху и снизу. Заодно в скоупированный файл добавлено объявление `--bz-*`: при автоматической выборке chrome-правил `:root` с высотами шапки не попал бы, и `.bz-header` осталась бы без высоты.

**Тесты:** `tests/Feature/SiteWideMenuTest.php` — 5 кейсов: шапка есть в каждом разделе (`/`, `/tenders`, `/companies`, `/projects`, `/news`, `/friends`), пункты прежних выпадающих меню доступны из меню раздела, старой Tailwind-навигации в разметке больше нет, гость видит то же меню на публичных страницах, состав пунктов на главной и внутренней странице совпадает.

**Изменённые файлы:** `resources/css/v26-chrome.css` (новый), `resources/views/partials/v26/{chrome,menu-items,services-items}.blade.php` (новые), `resources/views/layouts/app.blade.php`, `resources/views/layouts/navigation.blade.php` (удалён), `resources/views/home/{guest,authorized}.blade.php`, `resources/views/partials/v26/mobile-{guest,authorized}.blade.php`, `app/Http/Controllers/HomeController.php`, `vite.config.js`.

**Правка теста после удаления старой навигации.** `ProcurementMenuTest::test_user_without_company_does_not_see_organizer_items` рендерил `layouts.navigation` напрямую — этой вью больше нет. Тест переведён на общий партиал `partials.v26.menu-items`, проверка смысла не изменила: пользователь без компании не видит «Создать закупку» и «Мои закупки».

**Вторая правка теста.** `PriorityTasksTest::test_150_mobile_menu_has_collapsible_groups` проверял сворачиваемые группы прежнего мобильного меню на Alpine (`x-show="expanded"`). Это меню заменено шапкой v26, где панель открывается кнопкой, а пункты разбиты заголовками секций. Тест переименован в `test_150_mobile_menu_is_grouped_by_sections` и проверяет актуальную структуру — смысл задачи #150 (мобильное меню сгруппировано, а не «простыня») сохранён.

## #181 Фикс: съехало форматирование шапки на внутренних страницах (2026-08-30)

**Симптом (с теста).** На страницах разделов «форматирование меню и иконок съехало».

**Причина.** `v26-chrome.css` собирается из `v26.css` выборкой правил по именам классов шапки. Базовые правила эталона заданы глобально и под эту выборку не подпадают, поэтому в скоупированный файл не попали:

- `svg{fill:none;stroke:currentColor;stroke-width:1.8;…}` — без него иконки рисуются чёрной заливкой вместо обводки;
- `*{box-sizing:border-box}` — поехали размеры кнопок и полей шапки;
- `.page{…}` — панель теряла ширину и центрирование;
- шрифт и цвет текста эталона (на внутренних страницах Tailwind ставит свои).

**Что сделано.** Базовые правила дописаны в секцию PRODUCTION `v26-chrome.css` со скоупом `.v26-chrome`. У `.page` в шапке намеренно нет `min-height` эталонного кадра: она оборачивает только панель и выпадающие меню, иначе пустая шапка заняла бы весь экран.

**Тесты:** в `SiteWideMenuTest` добавлены два кейса. `test_every_chrome_class_has_styles` берёт классы отрендеренной шапки и требует правило в `v26-chrome.css` для каждого — ровно этот класс ошибок тестами разметки не ловится. `test_chrome_keeps_base_rules` держит на месте дописанные вручную базовые правила и объявление `--bz-*`.

**Изменённые файлы:** `resources/css/v26-chrome.css`, `tests/Feature/SiteWideMenuTest.php`.

## #181 Фикс: иконка закупок на мобильном и панели меню на планшете (2026-08-31)

**Замечания с теста:** в мобильной версии сломалась иконка «Закупки»; на планшете «поехали масштабы и прозрачность обоих меню».

**Иконка закупок.** Пункты «Найти закупку» и «Создать закупку» используют PNG-иконку, и в общем партиале меню класс был жёстко задан как `procurement-icon`. В мобильном скоупе эталона у неё другое имя и другой размер — `bz-procurement-icon` (21px против 27px), поэтому в мобильном меню картинка выпадала из строки. Класс иконки стал параметром партиала, мобильные включения передают `bz-procurement-icon`.

**Панели меню на планшете.** В эталоне `.auth-drawer` / `.guest-drawer` позиционируются `absolute` внутри `.page`, который там занимает весь экран. В шапке сайта `.page` оборачивает только саму панель, поэтому панель с `bottom:0` схлопывалась по высоте шапки, а полупрозрачная подложка не накрывала страницу — визуально это и выглядело как «съехавшие масштабы и прозрачность». Панели и подложка привязаны к вьюпорту (`position:fixed`) с отступом сверху под шапку: 76px на десктопе, 72px на планшете (там шапка ниже). Меню «Сервисы» получило `max-height` и прокрутку, чтобы длинный список не уходил за экран.

**Изменённые файлы:** `resources/css/v26-chrome.css`, `resources/views/partials/v26/{menu-items,chrome,mobile-authorized,mobile-guest}.blade.php`.

## #181 Правки по замечаниям заказчика от 30.08 и 03.09 (2026-09-06)

Закрыты все оставшиеся комментарии к #181: два от 30.08 (меню раздела, «Подписки», «Контакты»)
и три мобильных от 03.09.

**Мобильная версия (03.09).**

- **«Старое нижнее меню» на внутренних страницах.** Шапка рисовала свою нижнюю навигацию
  (Главная / Закупки / Компании / Профиль с подписями), а главная — навигацию эталона
  (поиск, сообщения, уведомления, помощь). Состав вынесен в общий партиал
  `partials/v26/mobile-bottom-nav.blade.php`, его же используют обе версии главной —
  разъехаться больше не могут.
- **Пустота между шапкой и контентом.** У эталонного `#bizzio-mobile-v1` задан
  `min-height:100dvh`: на главной он оборачивает весь экран телефона, а в шапке сайта все
  его дети (шапка, нижняя навигация, панели) позиционированы `fixed` — пустой контейнер
  просто отодвигал контент вниз на высоту экрана. В скоупе `.v26-chrome` `min-height`
  сброшен, а обёртка контента получила класс `.v26-page-body` (правило под неё в CSS уже
  было, но самого класса в разметке не было — контент ушёл бы под фиксированную шапку).
- **Иконка «Закупки» в мобильном меню «Сервисы».** PNG-иконка была жёстко задана классом
  `procurement-icon`; в мобильном скоупе эталона размеры есть только у `bz-procurement-icon`,
  поэтому картинка выпадала из плитки. Класс стал параметром партиала `services-items`
  (в `menu-items` это уже было сделано 31.08).

**Меню (30.08).**

- **Персонализация меню раздела.** Меню было одинаковым на всех страницах. Теперь пункты
  текущего сервиса выводятся отдельной группой сверху («Компании» → Каталог / Верифицированные /
  Создать, «Проекты» → Все / Активные / Создать, «Друзья» → Мои друзья / Входящие /
  Отправленные / Рекомендации, «Закупки» и «Новости» — как раньше), ниже идёт полное меню,
  чтобы при переходе между разделами ничего не пропадало. Активный пункт подсвечивается.
- **Полнота меню.** Добавлены «Подписки» (перенесены из правого меню «Сервисы» по замечанию
  от 30.08) и «Уведомления»; вкладки страницы «Друзья» доступны прямо из меню (`?tab=`).
- **«Контакты» → «Друзья».** Сервис переименован везде: плитки «Сервисов», карточки и
  заглушки на главной (десктоп и мобильная, гость и авторизованный).
- **Прокрутка панелей.** Меню стало длиннее — панели «Меню» (десктоп и мобильная) получили
  `overflow-y:auto`; правило добавлено и в `v26.css` (главная), и в `v26-chrome.css`
  (остальные страницы), иначе нижние пункты не достать.
- Попутно в `layouts/app.blade.php` убран лишний `</div>` (тег закрывался дважды) и добавлен
  `viewport-fit=cover` — мобильная шапка считает safe-area-инсеты.

**Тесты:** в `SiteWideMenuTest` четыре новых кейса — нижняя навигация одинакова на главной и
внутренних страницах и не содержит старых подписей; «Подписки» ушли из «Сервисов», а сервис
называется «Друзья»; меню персонализировано под каждый раздел и группа текущего сервиса идёт
первой; в мобильном меню «Сервисы» используется `bz-procurement-icon`. Заодно исправлена
граница выборки шапки в `test_every_chrome_class_has_styles`: маркер `<div class="v26-page-body">`
...
## 2026-09-09 — #181 в левом меню пропадали группы сервисов

**Симптом (замечание заказчика).** После релиза #319 в левой панели «Меню» стало заметно
меньше пунктов.

**Причина.** В `partials/v26/menu-items.blade.php` цикл, который дорисовывает «полное меню»
под группой текущего раздела, перебирал только две группы из пяти:

```php
foreach (['tenders', 'news'] as $key) {   // Компании, Проекты, Друзья пропущены
```

Поэтому группы **Компании**, **Проекты** и **Друзья** раскрывались только когда пользователь
находится внутри этого раздела; на всех остальных страницах от них оставалась одна ссылка
в группе «Моя работа». «Закупки» и «Новости» при этом всегда были полными — отсюда ощущение
неравномерно «похудевшего» меню.

Замер на стенде для авторизованного модератора до фикса: `/` — 15 пунктов, `/tenders` — 15,
`/companies` — 18 (раскрыта только группа «Компании»).

Это прямо противоречило заявленному в коммите bbe9ad2 поведению: «ниже — полное меню, чтобы
при переходе между разделами ничего не пропадало». Существующий тест
`test_section_menu_is_personalised_for_each_service` проверял только, что группа текущего
раздела присутствует и стоит первой, — про остальные группы он не спрашивал, поэтому дефект
и проехал в релиз.

**Фикс.** Цикл перебирает все пять групп; `isset` нужен из-за группы «Друзья» — её нет у гостя.

**Тесты:** два новых кейса в `SiteWideMenuTest` — `full_menu_below_keeps_every_service_on_any_page`
(все пять групп раскрыты на шести страницах, включая `/`) и `current_section_group_is_not_duplicated`
(группа текущего раздела не дублируется в полном меню ниже). Проверено, что на коде до фикса
первый тест падает.

**Изменённые файлы:** `resources/views/partials/v26/menu-items.blade.php`,
`tests/Feature/SiteWideMenuTest.php`, `docs/CHANGELOG_CLAUDE.md`.
