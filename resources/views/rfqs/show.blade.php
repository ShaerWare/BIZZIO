...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            {{-- #148: отмена запроса цен организатором до закрытия --}}
            @can('cancel', $rfq)
                <div x-data="{ showCancel: false }">
                    ...
                    <div x-show="showCancel" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                        <div class="bg-white rounded-lg shadow-xl p-5 w-full max-w-md" @click.outside="showCancel = false">
                            <h3 class="text-base font-semibold text-gray-900 mb-2">Отмена запроса цен</h3>
                            <p class="text-sm text-gray-600 mb-3">Укажите причину отмены — она будет видна участникам.</p>
                            <form method="POST" action="{{ route('rfqs.cancel', $rfq) }}">
                                @csrf
                                ...
                                <div class="mt-4 flex justify-end gap-2">
                                    ...
                                    <button type="submit"
                                            class="px-4 py-2 bg-red-600 text-white rounded-md text-sm hover:bg-red-700">Отменить запрос цен</button>
                                </div>
                            </form>
                        </div>
                    </div>
        ...
        @if($rfq->status === 'cancelled' && $rfq->cancellation_reason)
            <div class="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 mb-6">
                <strong>Запрос цен отменён.</strong> Причина: {{ $rfq->cancellation_reason }}
            </div>
        @endif
                ...
                <div class="flex flex-col md:flex-row gap-6">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex items-center text-gray-600">
                            ...
                            <span>Создатель: <strong>{{ $rfq->creator->full_name }}</strong></span>
                    ...
                    <div class="w-full md:w-80">
                        ...
                        @if($rfq->isCommercial())
                            <div class="bg-gray-50 rounded-lg p-4 mb-4">
                                ...
                                @include('partials.commercial-rating-rules', ['procedure' => $rfq])
                            </div>
                        @endif
...
@else
                    ...
                    @elseif($rfq->bids->count() > 0)
                        @php
                            ...
                            // Процедура завершена — раскрываем названия компаний
                            $canSeeNames = true;
                        @endphp
