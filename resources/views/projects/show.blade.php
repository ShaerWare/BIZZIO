                ...
                <div class="flex flex-col md:flex-row gap-6">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex items-center text-gray-600">
                            ...
                            <span>Создатель: <strong>{{ $project->creator->full_name }}</strong></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div id="content-description" class="tab-content">
                    @if($project->description)
                        <div class="prose max-w-none mb-6">
                            ...
                            <div class="text-gray-700">{!! nl2br(e($project->description)) !!}</div>
                        </div>
                    @endif
                    ...
                    @if($project->full_description)
                        <div class="prose max-w-none">
                            ...
                            <div class="text-gray-700">{!! nl2br(e($project->full_description)) !!}</div>
                        </div>
                    @endif
...
<script>
    ...
    /**
     * Включить режим редактирования комментария
     */
    function editComment(commentId) {
        // Скрываем текст комментария
        document.querySelector('.comment-body-' + commentId).classList.add('hidden');
        
        // Скрываем кнопки управления
        document.querySelector('.comment-actions-' + commentId).classList.add('hidden');
        
        // Показываем форму редактирования
        document.querySelector('.comment-edit-form-' + commentId).classList.remove('hidden');
    ...
    function cancelEdit(commentId) {
        ...
        document.querySelector('.comment-body-' + commentId).classList.remove('hidden');
        
        // Показываем кнопки управления
        document.querySelector('.comment-actions-' + commentId).classList.remove('hidden');
        
        // Скрываем форму редактирования
        document.querySelector('.comment-edit-form-' + commentId).classList.add('hidden');
    }

    /**
     * Обработка успешного редактирования (если используется AJAX)
     */
    function handleCommentUpdate(commentId, newBody) {
        // Обновляем текст комментария
        document.querySelector('.comment-body-' + commentId).textContent = newBody;
        
        // Отменяем режим редактирования
        cancelEdit(commentId);
        
        // Показываем уведомление (опционально)
        showNotification('Комментарий успешно обновлён!');
    }

    /**
     * Показать уведомление (простая реализация)
     */
    function showNotification(message) {
        // Простое alert-уведомление
        // Можно заменить на более красивое решение (toast, snackbar и т.д.)
    ...
    /**
     * AJAX-редактирование комментария (опционально)
     */
    document.addEventListener('DOMContentLoaded', function() {
        // Обработчик для всех форм редактирования комментариев
        document.querySelectorAll('[id^="edit-form-"]').forEach(form => {
            form.addEventListener('submit', function(e) {
                // Раскомментируй следующие строки, если хочешь использовать AJAX:
                ...
                // .then(data => {
                //     if (data.success) {
                //         handleCommentUpdate(commentId, data.comment.body);
                //     } else {
