...
- **Site-wide menu (#181).** Every page uses the v26 chrome: `layouts/app.blade.php` includes `partials/v26/chrome.blade.php` (top bar, «Меню» and «Сервисы» panels, mobile header and bottom nav). The old Tailwind `layouts/navigation.blade.php` is gone, and its second-level dropdowns («Закупки», «Новости») now live inside the section menu. Menu contents come from one place — `partials/v26/menu-items.blade.php` and `partials/v26/services-items.blade.php` — shared by the chrome, both home variants and the mobile partials; row/label classes are passed in because desktop and mobile markup of the prototype use different ones. The section menu is personalised: `menu-items` detects the current section from the route and renders that service's own items as the first group (Компании / Проекты / Друзья / Закупки / Новости), then the full menu below — «Подписки» live here, not in «Сервисы». The mobile bottom bar is `partials/v26/mobile-bottom-nav.blade.php`, shared by the chrome and both home variants (search / messages / notifications / help — four cells, as in the prototype). Chrome styles live in `resources/css/v26-chrome.css`, where every selector is scoped to `.v26-chrome`: the prototype's generic selectors would otherwise hit the Tailwind content of inner pages.
...
User → UserBadges (one-to-many; admin-assigned badges: colored border + label, shown on profile/RFQ/auction/revealed bids)
...
Rfq/Auction → ProcedureChatMessages (polymorphic MorphMany, #218)
Rfq/Auction → ProcedureParticipants (polymorphic MorphMany: chat_code + ban state, #218)
```
