...
- **Auction** — Real-time online trading, anonymized participants, configurable bid step
...
Events are registered in `AppServiceProvider@boot()`. Key events:
...
- `laravel/socialite` + `socialiteproviders/vkontakte` — OAuth (Google, VK)
...
- HTTPS is forced only when `APP_ENV=production` or `APP_URL` starts with https (see `AppServiceProvider`)
