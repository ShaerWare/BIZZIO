...
class AuctionController extends Controller
{
    ...
    private function commercialState(Auction $auction): array
    {
        ...
        // #237 Скрытые результаты прячут ход торгов только от ПОСТОРОННИХ. Организатор и
        // участники процедуры (этапа 2) всегда видят детали лидера и историю. Для постороннего
        // зрителя (публичный открытый аукцион) отдаём лишь целевой балл (best_score), детали скрыты.
        $hideResults = $auction->resultsHiddenFor(auth()->user());
